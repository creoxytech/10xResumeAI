...
import { generateResumeDesign } from "./services/gemini";
...
export default function App() {
  ...
  const submitMessage = useCallback(async (text) => {
    ...
    try {
      const newDesign = await generateResumeDesign(normalized, "", resumeDesign);
      setResumeDesign(newDesign);

      const aiMsg = {
        role: "assistant",
        text: "✅ Resume generated successfully! You can download it as PDF using the button above.",
      };
    ...
    return (
      <main className="min-h-screen bg-slate-50 dark:bg-slate-950 flex flex-col items-center justify-center p-4 relative overflow-hidden">
        ...
        <div className="z-10 w-full">
          ...
          <p className="text-center text-sm text-slate-400 mt-8 animate-fade-in">{status}</p>
        </div>
      </main>
    );
  }
